...
private static function prepareField($field_id, $condition, $base_class, $label ='') {
    ...
    $fieldset="<fieldset id='{$field_id}' {$class_list} {$data_name} {$condition_attributes}>";
    $start_id_div="<div id='{$field_id}' {$class_list} {$data_name} {$condition_attributes}>";
    $selectfield="<label for='{$field_id}' {$condition_attributes}>$label<select id='{$field_id}' name='{$field_id}' {$class_list} {$data_name} {$condition_attributes}>";
...
public static function generateToggleSwitch($field_id, $label, $value, $options_label = [], $condition = []) {
    ...
    return "{$fieldset}
            ...
            <div class='the-toggle' {$condition_attributes}>
                <input type='radio' name='{$field_id}' {$data_name} value='1' " . checked($value, "1", false) . " class='toggle-input toggle-yes' id='{$field_id}_yes'>
                <input type='radio' name='{$field_id}' {$data_name} value='0' " . checked($value, "0", false) . " class='toggle-input toggle-no' id='{$field_id}_no'>
...
public static function generateSelect($field_id, $label, $value, $options, $condition = []) {
    extract(self::prepareField($field_id, $condition, "dibraco-select", $label));
...
public static function generateNoEditField($field_id, $label, $value, $condition = []) {
    extract(self::generateSimpleField($field_id, "no-edit", $value, $condition, $field_config=[]));
   return "{$input_string}";
}
...
public static function generateHiddenField($field_id, $value, $condition = []) {
    ...
    return "<div {$class_string} {$condition_attributes} style='display: none;'>
                <input type='hidden' id='{$field_id}' name='{$field_id}' {$data_name} value='{$value}'>
            </div>";
}
...
if ($rows ===''){
    $field_config['rows'] = 6;
        ...
        $field_config['rows'] = $rows;
    }
    extract(self::generateSimpleField($field_id, "wysiwyg", $value, $condition, $field_config));
    return "{$div_with_textarea}";
}
...
if ($rows ===''){
    ...
    extract(self::generateSimpleField($field_id, "textarea", $value, $condition, $field_config));
    return "{$div_with_textarea}";
}
public static function generateTimeInput($field_id, $label, $value, $condition = []) {
    extract(self::generateSimpleField($field_id, "time", $value, $condition, $field_config=[]));
    return "{$input_string}";
}
public static function generateNumberInput($field_id, $label, $value, $step = 1, $condition = []) {
    $field_config['step'] = $step;
    extract(self::generateSimpleField($field_id, "number", $value, $condition, $field_config));
   return "{$input_string}";
}
public static function generateDateInput($field_id, $label, $value, $condition = []) {
    extract(self::generateSimpleField($field_id, "date", $value, $condition, $field_config=[]));
    return "{$input_string}";
}
public static function generateTextInput($field_id, $label, $value, $condition = []) {
   extract(self::generateSimpleField($field_id, "text", $value, $condition, $field_config=[]));
 return "{$input_string}";
}
...
private static function generateSimpleField($field_id, $field_type, $value, $condition, $field_config=[]){
 ...
 if ($field_type==='textarea' || $field_type==='wysiwyg'){
      ...
      $start_div = "<div {$class_list} {$data_name} {$condition_attributes}>";
      return ['div_with_textarea' => "{$start_div}{$label_for}<textarea {$string_name} {$string_id} {$rows_string} {$data_name}>$value</textarea></div>"];
 }
 if($field_type ==='number'){
    $step = $field_config['step'];
    $string_step = "step='{$step}'";
    return ['input_string' => "{$label_string}<input {$string_type} {$string_id} {$string_name} {$string_value} {$string_step} {$data_name} {$class_list} {$condition_attributes}></label>"];
 }
 if($field_type === 'no-edit'){
     ['input_string'=> "{$label_string}<p {$string_id} {$string_name} {$string_value} {$class_list} {$condition_attributes}></label>"];
 }
 return ['input_string'=> "{$label_string}<input {$string_type} {$string_id} {$string_name} {$string_value} {$class_list} {$condition_attributes}></label>"];
...
public static function generateImageField($field_id, $label, $value, $condition = []) {
  ...
  if ($value !== '') {
    ...
    return"{$start_div}{$title}
                    <img class='image-preview' src='{$value}' id='{$name_normalized}-preview' image_id='{$image_id}' style='max-width: 100%; object-fit: contain; aspect-ratio: auto; height: auto; display: block;'>
                        ...
                        <div class='buttons'>
                            ...
                            <button type='button' class='button dibraco_clear_image_button' data-name='{$name_normalized}-clear-image-button' data-input='{$name_normalized}' data-preview='{$name_normalized}-preview' aria-controls='{$name_normalized}-input'>Clear Image</button>
                        </div>
                </div>";
}
    ...
    if ($type === "injectionend") {
      ...
      if (empty($field_name) || empty($type) || empty($field_config)) {
        ...
        switch ($type) {
            ...
            case 'no-edit':
                return self::generateNoEditField($field_name, $label, $value, $condition);
            case 'hidden': 
...
public static function generateVisualSection($field_name, $field_config, $is_recursive = false) {
    ...
    return "{$start_id_div}
        {$hidden['storage_field']}
        <div class='section-fields {$field_name}'>
        {$subfields_html}
...
public static function generateRepeaterField($field_name, $field_config, $parent_name = null) {
    ...
    if (!array_key_exists('row_count', $field_config) || (!array_key_exists("{$field_name}_row_count", $field_config))) {
        $row_count=1;
    ...
    extract(self::prepareField($field_name, $condition, "dibraco-repeater-wrapper", $label));
    $row_html = '';
     ...
     foreach ($row_config as $subfield_name => $subfield_config) {
                ...
                default:
                   $sublabel = ucwords(str_replace(['_', '-'], ' ', $subfield_name));
                    ...
                    if (self::$tracking_started) {
                    if (isset(self::$meta_array[$field_name][$index][$subfield_name])) {
                        $subfield_config['value'] = self::$meta_array[$field_name][$index][$subfield_name];
                    }
                    elseif (isset(self::$meta_array[$subfield_name_with_index])) {
                    $subfield_config['value'] = self::$meta_array[$subfield_name_with_index];
                    }
                    }
    ...
    return "<div id='{$field_name}' {$class_string} {$condition_attributes} {$data_name} {$parent_name}>
     <h4>{$label}</h4>
       <input type='hidden' name='{$field_name}_row_count' class='repeater-input' value='{$row_count}' {$data_name} {$parent_name}>
           ...
           <button type='button' class='add-row-button' {$parent_name} {$data_name}>Add Row</button></div><input type='hidden' name='{$field_name}_end'>";
}
public static function generateGroup($field_name, $field_config) {
    ...
    extract(self::prepareField($field_name, $condition, "group-fields"));
    if ($label !==null) {
        $label = ucwords(str_replace(['_', '-'], ' ', $label));
        $label = "<h4>{$label}</h4>";
    }
