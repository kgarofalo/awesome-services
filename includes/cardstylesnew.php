...
function render_card_styles_selection_page() {
    $enabled_contexts = get_option('enabled_context_names');
    $selected_contexts = get_option('selected_contexts', []);
    
    $toggle_fields = [];
    foreach ($enabled_contexts as $context_name => $context_label) {
        $toggle_fields[$context_name] = [
            'type' => 'toggle',
            'value' => in_array($context_name, $selected_contexts) ? '1' : '0',
    ...
    <div class="wrap">
        ...
        <form method="POST">
            <?php echo FormHelper::generateVisualFieldGroup('card_styles', ['type' => 'visual_group', 'fields' => $toggle_fields]); ?>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $new_selected = [];
        foreach ($enabled_contexts as $context_name => $label) {
            if ($_POST[$context_name] === '1') {
                $new_selected[] = $context_name;
            }
        }
        update_option('selected_contexts', $new_selected);
    }
}
